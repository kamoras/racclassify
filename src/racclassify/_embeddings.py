...
EncoderFn = Callable[[list[str]], npt.NDArray[Any]]
...
def embed_batch(
    ...
    *,
    ...
    embs: npt.NDArray[Any] = np.asarray(raw, dtype=np.float32)
    norms = np.linalg.norm(embs, axis=1, keepdims=True)
    norms[norms == 0] = 1.0
    return embs / norms  # type: ignore[no-any-return]
...
def embed_one(
    ...
    *,
    ...
    return embed_batch(  # type: ignore[no-any-return]
        [text], model_name=model_name, encoder=encoder, max_chars=max_chars
    )[0]
